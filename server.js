...
const users = result.rows;  let changed = false;
...
const result = await pool.query("SELECT * FROM users");
const users = result.rows;  const index = users.findIndex((item) => item.id === req.session.user.id);
...
const result = await pool.query("SELECT * FROM users");
const users = result.rows;  const target = users.find((item) => item.id === req.params.id);
